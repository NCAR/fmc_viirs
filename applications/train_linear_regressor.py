...
def trainer(conf, trial = False, verbose = True):
    ...
    for k_fold, (train_data, valid_data, test_data) in enumerate(data_folds):
        ...
        lg_pred_train = scaler_y.inverse_transform(model.predict(x_train))
        lg_pred_valid = scaler_y.inverse_transform(model.predict(x_valid))
        lg_pred_test = scaler_y.inverse_transform(model.predict(x_test))

        results_dict["train_rmse"].append(mean_squared_error(y_train, lg_pred_train)**(1/2))
        results_dict["train_mae"].append(mean_absolute_error(y_train, lg_pred_train))
        results_dict["train_r2"].append(r2_score(y_train, lg_pred_train))
        results_dict["valid_rmse"].append(mean_squared_error(y_valid, lg_pred_valid)**(1/2))
        results_dict["valid_mae"].append(mean_absolute_error(y_valid, lg_pred_valid))
        results_dict["valid_r2"].append(r2_score(y_valid, lg_pred_valid))
        results_dict["test_rmse"].append(mean_squared_error(y_test, lg_pred_test)**(1/2))
        results_dict["test_mae"].append(mean_absolute_error(y_test, lg_pred_test))
        results_dict["test_r2"].append(r2_score(y_test, lg_pred_test))
        models.append([scaler_x, scaler_y, model])
